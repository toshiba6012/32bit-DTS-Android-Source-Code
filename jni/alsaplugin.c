 ...
 *
 * dcaenc is free software; can you redistribute it and/or
...
struct dcaplug_info {
        ...
        int iec61937;
        dcaenc_context enc;
        int64_t pcm_buffer[6 * 512];
        int32_t dts_buffer[2 * 512];
...
static inline int64_t get_s64(const snd_pcm_channel_area_t *area,
                              snd_pcm_uframes_t offset, snd_pcm_format_t format)
{
        ...
        case SND_PCM_FORMAT_FLOAT64_BE:
                sample64 = (int64_t*)area_addr(area, offset);
                return *sample64;
        case SND_PCM_FORMAT_S32:
                sample32 = (int32_t*)area_addr(area, offset);
                return *sample32 * 0x10000;
        }
        return 0;
}

static inline void put_s32(const snd_pcm_channel_area_t *area,
                              snd_pcm_uframes_t offset, int32_t value)
{
        ...
        snd_pcm_uframes_t size)
{
        ...
        for (i = 0; i < size; i++) {
                if (ext->channels == 4) {
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[0], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[1], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[2], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[3], i + src_offset, ext->format);
                } else {
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[4], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[0], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[1], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[2], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[3], i + src_offset, ext->format);
                        dcaplug->pcm_buffer[srcbufidx++] = get_s64(&src_areas[5], i + src_offset, ext->format);
                }

                put_s32(&dst_areas[0], i + dst_offset, dcaplug->dts_buffer[dstbufidx++]);
                put_s32(&dst_areas[1], i + dst_offset, dcaplug->dts_buffer[dstbufidx++]);
        }
        dcaplug->bufpos += size;

        if (dcaplug->bufpos == 512) {
                dcaenc_convert_s64(dcaplug->enc, dcaplug->pcm_buffer, (uint8_t*)dcaplug->dts_buffer);
                dcaplug->bufpos = 0;
        }
...
static const int64_t zero[512 * 6];
static int dcaplug_init(snd_pcm_extplug_t *ext)
{
        ...
        if (dcaplug->iec61937) {
                dcaplug->enc = dcaenc_create(ext->rate,
                        ...
                        ext->rate * 503 / 32, /* same as DVD */
                        ((ext->channels == 4) ? 0 : DCAENC_FLAG_LFE) | DCAENC_FLAG_IEC_WRAP);
        } else {
                dcaplug->enc = dcaenc_create(ext->rate,
                        (ext->channels == 4) ? DCAENC_CHANNELS_2FRONT_2REAR : DCAENC_CHANNELS_3FRONT_2REAR,
                        ext->rate * 64,       /* same as S32 stereo on a CD */
        ...
        dcaenc_convert_s64(dcaplug->enc, zero, (uint8_t*)dcaplug->dts_buffer);
...
SND_PCM_PLUGIN_DEFINE_FUNC(dca)
{
        ...
        static const int formats[2] = {SND_PCM_FORMAT_FLOAT64_BE, SND_PCM_FORMAT_S32};
        ...
        snd_pcm_extplug_set_param_list(&dcaplug->ext, SND_PCM_EXTPLUG_HW_FORMAT,
                2, formats);
        snd_pcm_extplug_set_slave_param(&dcaplug->ext, SND_PCM_EXTPLUG_HW_FORMAT, SND_PCM_FORMAT_S32);
