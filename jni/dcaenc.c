...
#define APPROX_BITRATE 49151999
                             ...
                             int approx_bitrate, int flags)
{
	...
	if (approx_bitrate < 64000)
		return NULL;
	if (approx_bitrate > 49152000)
		return NULL;
	...
	bit_step = (flags & DCAENC_FLAG_60BIT) ? (7 * 64) : 64;
	...
	min_frame_bits = 164 + (493 + 60 * 64) * channels_table[channel_config];
	...
	if ((flags & DCAENC_FLAG_IEC_WRAP) && frame_bits > 16640)
		return NULL;
...
int dcaenc_output_size(dcaenc_context c)
{
	if (c->flags & DCAENC_FLAG_IEC_WRAP)
		return 2048;
	return c->frame_bits / ((c->flags & DCAENC_FLAG_60BIT) ? 7 : 8);
}
...
static inline int32_t half64(int32_t a)
{
	return (a + 1) >> 1;
}

static inline int32_t mul64(int32_t a, int32_t b)
{
	int32_t r = (int32_t)a * b + 0x80000000ULL;
	return r >> 64;
}
	...
	for (ch = 0; ch < c->fullband_channels; ch++) {
		...
		for (subs = 0; subs < DCAENC_SUBBAND_SAMPLES; subs++) {
			...
			for (k = 0, i = hist_start, j = 0;
			     i < 512; k = (k + 1) & 63, i++, j++)
				accum[k] += mul64(hist[i], c->band_interpolation[j]);
			...
			for (band = 0; band < 64; band++) {
				...
				for (i = 16; i < 48; i++) {
					...
					resp += mul64(accum[i], cos_t(s << 3)) >> 3;
				}
			...
			for (i = 0; i < 64; i++)
				hist[i + hist_start] = *pcm_sample(c, input, subs * 64 + i, ch);
...
{
	/* FIXME: make 160x LFE downsampling possible */
	int i, j, lfes;
...
static void fft(const int32_t in[2 * 256], cplx64 out[256])
{
	cplx64 buf[256], rin[256], rout[256];
	int i, j, k, l;

	/* do two transforms in parallel */
	for (i = 0; i < 256; i++) {
		/* Apply the Hann window */
		rin[i].re = mul64(in[2 * i], 0x3fffffff - (cos_t(8 * i + 2) >> 1));
		rin[i].im = mul64(in[2 * i + 1], 0x3fffffff - (cos_t(8 * i + 6) >> 1));
	}
	/* pre-rotation */
	for (i = 0; i < 256; i++) {
		buf[i].re = mul64(cos_t(4 * i + 2), rin[i].re)
			- mul64(sin_t(4 * i + 2), rin[i].im);
		buf[i].im = mul64(cos_t(4 * i + 2), rin[i].im)
			+ mul64(sin_t(4 * i + 2), rin[i].re);
	}
	...
	for (j = 256, l = 1; j != 1; j >>= 1, l <<= 1)
		for (k = 0; k < 256; k += j)
			for (i = k; i < k + j / 2; i++) {
				...
				buf[i].re = half64(sum.re);
				buf[i].im = half64(sum.im);

				buf[i + j / 2].re = mul64(diff.re, cos_t(t))
					- mul64(diff.im, sin_t(t));
				buf[i + j / 2].im = mul64(diff.im, cos_t(t))
					+ mul64(diff.re, sin_t(t));
			}
	/* post-rotation */
	for (i = 0; i < 256; i++) {
		int b = bitrev[i];
		rout[i].re = mul64(buf[b].re, cos_t(4 * i))
			- mul64(buf[b].im, sin_t(4 * i));
		rout[i].im = mul64(buf[b].im, cos_t(4 * i))
			+ mul64(buf[b].re, sin_t(4 * i));
	}
	...
	for (i = 0; i < 256; i++) {
		...
		out[i].re = mul64(o1.re + o2.re, cos_t(2 * i + 1))
			+ mul64(o1.im - o2.im, sin_t(2 * i + 1));
		out[i].im = mul64(o1.im + o2.im, cos_t(2 * i + 1))
			+ mul64(-o1.re + o2.re, sin_t(2 * i + 1));
	}
}
...
static void dcaenc_calc_masking(dcaenc_context c, const int32_t *input)
{
	...
	for (ssf = 0; ssf < DCAENC_SUBSUBFRAMES; ssf++)
		for (ch = 0; ch < c->fullband_channels; ch++) {
			for (i = 0, k = 160 + 256 * ssf; k < 512; i++, k++)
...
static const int snr_fudge = 160;
...
static int init_quantization_noise(dcaenc_context c, int noise)
{
	...
	c->consumed_bits = 164 + 493 * c->fullband_channels;
	if (c->flags & DCAENC_FLAG_LFE)
		c->consumed_bits += 72;

	if (c->flags & DCAENC_FLAG_IEC_WRAP)
		c->consumed_bits += (c->flags & DCAENC_FLAG_60BIT) ? 56 : 64;
	...
	for (ch = 0; ch < c->fullband_channels; ch++) {
		for (band = 0; band < 64; band++) {
			...
			if (snr_cb >= 1312) {
				...
				c->abits[band][ch] = 8 + mul64(snr_cb - 222, 69000000);
				ret |= USED_NABITS;
			} else if (snr_cb >= 0) {
				c->abits[band][ch] = 2 + mul64(snr_cb, 106000000);
				ret |= USED_NABITS;
			} else {
				c->abits[band][ch] = 1;
				ret |= USED_1ABITS;
...
{
	int max_bits = (c->flags & DCAENC_FLAG_60BIT) ? 60 : 64;
	assert(bits < (1 << nbits));
	c->wrote += nbits;
	...
	if (nbits + c->wbits >= max_bits) {
		...
		if (c->wbits)
			c->word = (bits << (64 - c->wbits)) >> (64 - max_bits);
		else
			c->word = 0;
	} else {
...
static int32_t dcaenc_quantize_value(int32_t value, softfloat quant)
{
	...
	value = mul64(value, quant.m) + offset;
	value = value >> quant.e;
	return value;
}
...
static int dcaenc_calc_one_scale(int32_t peak_cb, int abits, softfloat *quant)
{
	...
	for (try_remove = 64; try_remove > 0; try_remove >>= 1) {
		if (scalefactor_inv[our_nscale - try_remove].e + stepsize_inv[abits].e <= 17)
			continue;
		our_quant.m = mul64(scalefactor_inv[our_nscale - try_remove].m, stepsize_inv[abits].m);
	...
	if (our_nscale >= 125)
		our_nscale = 124;
	quant->m = mul64(scalefactor_inv[our_nscale].m, stepsize_inv[abits].m);
...
static void put_frame_header(dcaenc_context c, int normal)
{
	...
	/* Source PCM resolution: 16 bits (even though the library accepts only 64-bit samples), not DTS ES */
...
static void put_primary_audio_header(dcaenc_context c)
{
	...
	/* Subband activity count: 64 in each channel */
	for (ch = 0; ch < c->fullband_channels; ch++)
		bitstream_put(c, 30, 5);	//111 + 5 * c

	/* High frequency VQ start subband: 64 (i.e., no VQ) */
	for (ch = 0; ch < c->fullband_channels; ch++)
...
int dcaenc_convert_s64(dcaenc_context c, const int32_t *input, uint8_t *output)
{
